...
@section('content')
    <div class="container-fluid" style="color: black;">
                        ...
                        @foreach ($announcements as $announcement)
                            <li class="list-group-item">
                                ...
                                <div class="text-muted d-flex flex-column mb-3">
                                    ...
                                    <small>Date Issued:
                                        {{ date('M d, Y \a\t h:i A', strtotime($announcement->created_at)) }}</small>
                                </div>
                                ...
                                @if (Auth::user()->user_role == 1)
                                    <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                                        ...
                                        <form action="/admin/announcements/disapprove/{{ $announcement->id }}"
                                            method="POST">
                                            @csrf
                                            @method('post')
                                ...
                                @elseif (Auth::user()->user_role >= 4)
                                    <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                                        ...
                                        <form action="/user/announcements/disapprove/{{ $announcement->id }}"
                                            method="POST">
                                            @csrf
                                            @method('post')
                                            <button class="btn btn-danger btn-icon-split">
        ...
        @else
            <div class="card">
                <div class="card-body">
                    There are no pending announcements yet.
                </div>
            </div>
        @endif
    </div>
